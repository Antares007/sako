...
void drawWindow(NVGcontext *vg, const char *title, float x, float y, float w,
                float h) {
...
void drawSearchBox(NVGcontext *vg, const char *text, float x, float y, float w,
                   float h) {
  NVGpaint bg;
  char icon[8];
  ...
  // Edit
  bg = nvgBoxGradient(vg, x, y + 1.5f, w, h, h / 2, 5, nvgRGBA(0, 0, 0, 16),
                      nvgRGBA(0, 0, 0, 92));
  nvgBeginPath(vg);
  nvgRoundedRect(vg, x, y, w, h, cornerRadius);
  nvgFillPaint(vg, bg);
  ...
  nvgFillColor(vg, nvgRGBA(255, 255, 255, 32));
  nvgTextAlign(vg, NVG_ALIGN_CENTER | NVG_ALIGN_MIDDLE);
  nvgText(vg, x + w - h * 0.55f, y + h * 0.55f,
          cpToUTF8(ICON_CIRCLED_CROSS, icon), NULL);
}

void drawDropDown(NVGcontext *vg, const char *text, float x, float y, float w,
                  float h) {
  NVGpaint bg;
  char icon[8];
  float cornerRadius = 4.0f;

  bg = nvgLinearGradient(vg, x, y, x, y + h, nvgRGBA(255, 255, 255, 16),
                         nvgRGBA(0, 0, 0, 16));
  nvgBeginPath(vg);
  ...
  nvgFillColor(vg, nvgRGBA(255, 255, 255, 64));
  nvgTextAlign(vg, NVG_ALIGN_CENTER | NVG_ALIGN_MIDDLE);
  nvgText(vg, x + w - h * 0.5f, y + h * 0.5f,
          cpToUTF8(ICON_CHEVRON_RIGHT, icon), NULL);
}

void drawLabel(NVGcontext *vg, const char *text, float x, float y, float w,
               float h) {
  NVG_NOTUSED(w);
...
void drawEditBoxBase(NVGcontext *vg, float x, float y, float w, float h) {
  NVGpaint bg;
  // Edit
  bg = nvgBoxGradient(vg, x + 1, y + 1 + 1.5f, w - 2, h - 2, 3, 4,
...
void drawEditBox(NVGcontext *vg, const char *text, float x, float y, float w,
                 float h) {
...
void drawCheckBox(NVGcontext *vg, const char *text, float x, float y, float w,
                  float h) {
  NVGpaint bg;
  char icon[8];
  NVG_NOTUSED(w);
  ...
  bg = nvgBoxGradient(vg, x + 1, y + (int)(h * 0.5f) - 9 + 1, 18, 18, 3, 3,
...
void drawButton(NVGcontext *vg, int preicon, const char *text, float x, float y,
  ...
  if (preicon != 0) {
    ...
    nvgTextAlign(vg, NVG_ALIGN_LEFT | NVG_ALIGN_MIDDLE);
    nvgText(vg, x + w * 0.5f - tw * 0.5f - iw * 0.75f, y + h * 0.5f,
            cpToUTF8(preicon, icon), NULL);
  }
...
void drawSlider(NVGcontext *vg, float pos, float x, float y, float w, float h) {
  ...
  // Slot
  bg = nvgBoxGradient(vg, x, cy - 2 + 1, w, 4, 2, 2, nvgRGBA(0, 0, 0, 32),
                      nvgRGBA(0, 0, 0, 128));
  nvgBeginPath(vg);
  ...
  nvgBeginPath(vg);
  nvgRect(vg, x + (int)(pos * w) - kr - 5, cy - kr - 5, kr * 2 + 5 + 5,
          kr * 2 + 5 + 5 + 3);
  ...
  // Knob
  knob = nvgLinearGradient(vg, x, cy - kr, x, cy + kr,
...
void drawGraph(NVGcontext *vg, float x, float y, float w, float h, float t) {
  ...
  for (i = 1; i < 6; i++)
    nvgBezierTo(vg, sx[i - 1] + dx * 0.5f, sy[i - 1], sx[i] - dx * 0.5f, sy[i],
                sx[i], sy[i]);
  ...
  for (i = 1; i < 6; i++)
    nvgBezierTo(vg, sx[i - 1] + dx * 0.5f, sy[i - 1] + 2, sx[i] - dx * 0.5f,
                sy[i] + 2, sx[i], sy[i] + 2);
  nvgStrokeColor(vg, nvgRGBA(0, 0, 0, 32));
  ...
  // Graph sample pos
  for (i = 0; i < 6; i++) {
    bg = nvgRadialGradient(vg, sx[i], sy[i] + 2, 3.0f, 8.0f,
...
void drawSpinner(NVGcontext *vg, float cx, float cy, float r, float t) {
  ...
  paint = nvgLinearGradient(vg, ax, ay, bx, by, nvgRGBA(0, 0, 0, 0),
                            nvgRGBA(0, 0, 0, 128));
...
void drawThumbnails(NVGcontext *vg, float x, float y, float w, float h,
  ...
  // Drop shadow
  shadowPaint = nvgBoxGradient(vg, x, y + 4, w, h, cornerRadius * 2, 20,
                               nvgRGBA(0, 0, 0, 128), nvgRGBA(0, 0, 0, 0));
  nvgBeginPath(vg);
  nvgRect(vg, x - 10, y - 10, w + 20, h + 30);
  nvgRoundedRect(vg, x, y, w, h, cornerRadius);
  ...
  for (i = 0; i < nimages; i++) {
    ...
    imgPaint = nvgImagePattern(vg, tx + ix, ty + iy, iw, ih,
                               0.0f / 180.0f * NVG_PI, images[i], a);
    nvgBeginPath(vg);
    nvgRoundedRect(vg, tx, ty, thumb, thumb, 5);
    nvgFillPaint(vg, imgPaint);
    nvgFill(vg);

    shadowPaint = nvgBoxGradient(vg, tx - 1, ty, thumb + 2, thumb + 2, 5, 3,
                                 nvgRGBA(0, 0, 0, 128), nvgRGBA(0, 0, 0, 0));
    nvgBeginPath(vg);
  ...
  // Hide fades
  fadePaint = nvgLinearGradient(vg, x, y, x, y + 6, nvgRGBA(200, 200, 200, 255),
  ...
  fadePaint =
      nvgLinearGradient(vg, x, y + h, x, y + h - 6, nvgRGBA(200, 200, 200, 255),
                        nvgRGBA(200, 200, 200, 0));
  nvgBeginPath(vg);
  ...
  // Scroll bar
  shadowPaint = nvgBoxGradient(vg, x + w - 12 + 1, y + 4 + 1, 8, h - 8, 3, 4,
                               nvgRGBA(0, 0, 0, 32), nvgRGBA(0, 0, 0, 92));
  nvgBeginPath(vg);
  ...
  shadowPaint = nvgBoxGradient(
      vg, x + w - 12 - 1, y + 4 + (h - 8 - scrollh) * u - 1, 8, scrollh, 3, 4,
      nvgRGBA(220, 220, 220, 255), nvgRGBA(128, 128, 128, 255));
  nvgBeginPath(vg);
  nvgRoundedRect(vg, x + w - 12 + 1, y + 4 + 1 + (h - 8 - scrollh) * u, 8 - 2,
                 scrollh - 2, 2);
  nvgFillPaint(vg, shadowPaint);
  //	nvgFillColor(vg, nvgRGBA(0,0,0,128));
  nvgFill(vg);
...
void drawColorwheel(NVGcontext *vg, float x, float y, float w, float h,
                    float t) {
  int i;
  ...
  paint = nvgBoxGradient(vg, r0 - 3, -5, r1 - r0 + 6, 10, 2, 4,
                         nvgRGBA(0, 0, 0, 128), nvgRGBA(0, 0, 0, 0));
  nvgBeginPath(vg);
  ...
  paint = nvgRadialGradient(vg, ax, ay, 7, 9, nvgRGBA(0, 0, 0, 64),
                            nvgRGBA(0, 0, 0, 0));
  nvgBeginPath(vg);
...
void drawParagraph(NVGcontext *vg, float x, float y, float width, float height,
                   float mx, float my) {
  NVGtextRow rows[3];
  NVGglyphPosition glyphs[100];
  const char *text =
      "This is longer chunk of text.\n  \n  Would have used lorem ipsum but "
      "she    was busy jumping over the lazy dog with the fox and all the men "
  ...
  while ((nrows = nvgTextBreakLines(vg, start, end, width, rows, 3))) {
    for (i = 0; i < nrows; i++) {
      ...
      if (hit) {
        ...
        px = x;
        nglyphs =
            nvgTextGlyphPositions(vg, x, y, row->start, row->end, glyphs, 100);
        for (j = 0; j < nglyphs; j++) {
          float x0 = glyphs[j].x;
  ...
  if (gutter) {
    ...
    nvgBeginPath(vg);
    nvgFillColor(vg, nvgRGBA(255, 192, 0, 255));
    nvgRoundedRect(vg, (int)bounds[0] - 4, (int)bounds[1] - 2,
  ...
  nvgTextBoxBounds(
      vg, x, y, 150,
      "Hover your mouse over the text to see calculated caret position.", NULL,
      bounds);
...
static void setAlpha(unsigned char *image, int w, int h, int stride,
                     unsigned char a) {
  int x, y;
  for (y = 0; y < h; y++) {
    unsigned char *row = &image[y * stride];
