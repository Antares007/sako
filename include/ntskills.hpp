...
namespace std {
template <typename T, typename Parameter, template <typename> class... Skills>
struct hash<nt::NamedType<T, Parameter, Skills...>> {
  using NamedType = nt::NamedType<T, Parameter, Skills...>;
  using checkIfHashable =
      typename std::enable_if<NamedType::is_hashable, void>::type;

  size_t operator()(nt::NamedType<T, Parameter, Skills...> const &x) const {
