...
template <typename Function> void performAction(Comparator<Function> comp) {
  auto a = comp.get();
  a(1);
}

struct bark {
  enum entrytype { blob, tree, link, commit };
  struct entry {
    std::string name;
    entrytype type;
    void (*a)(void);
  };
  using Pith = void (*)(std::function<void(int)>);
  int operator()(Pith pith) const {
    int sum = 0;
    pith([&](int r) { sum = sum + r; });
    return sum;
  };
};

int main() {
  auto aa = bark::entry{"hello", bark::blob, []() { auto x = 1; }};
  aa.a();
  auto rez = bark{}([](auto o) {
    o(bark{}([](auto o) {
      o(1);
      o(2);
      o(3);
  ...
  exit(0);
  auto x =
      fluent::make_named<Comparator>([](int x) { std::cout << "compare\n"; });
  performAction(x);
